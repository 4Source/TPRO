...
LightEffectManager::LightEffectManager(ConfigManager *config_manager,
                                       TimeApi *time_api)
    : config_manager(config_manager), time_api(time_api) {}
...
esp_err_t LightEffectManager::register_effect(Effect *effect) {
  ...
  auto it = std::find(available_effects.begin(), available_effects.end(), effect);
  if (it != available_effects.end()) {
...
esp_err_t LightEffectManager::unregister_effect(Effect *effect) {
  ...
  available_effects.erase(it);
  return ESP_OK;
}

Effect *LightEffectManager::get_effect() const {
...
const LedFrame &LightEffectManager::get_led_data() const {
  return data;
}
...
void LightEffectManager::set_time_api(TimeApi *api) {
  time_api = api;
}
...
void LightEffectManager::set_speed(float new_speed) {
  speed = new_speed;
}

float LightEffectManager::get_speed() const {
  return speed;
}