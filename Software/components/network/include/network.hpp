...
/**
 * Start the network connections Wi-Fi and/or Ethernet. Should be configured first with init_network.
 *
 * @retval - `ESP_OK`: Succeed
 * @retval - `ESP_ERR_WIFI_NOT_CONNECT`: Wifi failed to connect to AP
 * @retval - `ESP_ERR_WIFI_NOT_INIT`: WiFi is not initialized by esp_wifi_init
 * @retval- `ESP_ERR_INVALID_ARG`: It doesn't normally happen, the function called inside the API was passed invalid argument, user should check if
 * the WiFi related config is correct
