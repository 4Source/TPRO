...
httpd_handle_t Webserver::g_http_server_handle = nullptr;
...
void Webserver::connect_handler(void *arg_0, esp_event_base_t event_base, int32_t event_id, void *arg_1) {
	...
	// http server (for automatic redirect to https)
	httpd_config_t http_config = HTTPD_DEFAULT_CONFIG();
	http_config.uri_match_fn = httpd_uri_match_wildcard;

	if (httpd_start(&g_http_server_handle, &http_config) == ESP_OK) {
		register_http_routes(g_http_server_handle);
	}
...
void Webserver::disconnect_handler(void *arg_0, esp_event_base_t event_base, int32_t event_id, void *arg_1) {
	...
	if (g_http_server_handle != nullptr) {
		httpd_ssl_stop(g_http_server_handle);
		g_http_server_handle = nullptr;
	}
}
...
void Webserver::event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
	if (event_base == ESP_HTTPS_SERVER_EVENT && event_id == HTTPS_SERVER_EVENT_ERROR) {
		handle_server_error(static_cast<esp_https_server_last_error_t *>(event_data));
	}
}

void Webserver::handle_server_error(esp_https_server_last_error_t *event) {
